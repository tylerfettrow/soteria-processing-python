...
for this_crew in crews_to_process:
	...
	for i_scenario in range(len(scenarios)):
		for i_devicefile in range(len(file_types)):
			process_dir_name = crew_dir + "/Analysis/"
	...
	os.rmdir(crew_dir + "/Analysis/" + 'file_existence_matrix/file_existence_matrix.npy')
	np.save(crew_dir + "/Analysis/" + 'file_existence_matrix',file_existence_matrix)
