    ...
    for i_seat in range(len(file_types)):
        for i_scenario in range(len(scenarios)):
            ...
            if blob.exists():
                ...
                eeg_freqSpec_band_storage[1, :, 0 : Sxx.shape[2], i_scenario] = Sxx[
                    :, 0:3, :
                ].mean(
                    1
                )  # delta
                eeg_freqSpec_band_storage[2, :, 0 : Sxx.shape[2], i_scenario] = Sxx[
                    :, 4:7, :
                ].mean(
                    1
                )  # theta
                eeg_freqSpec_band_storage[3, :, 0 : Sxx.shape[2], i_scenario] = Sxx[
                    :, 8:12, :
                ].mean(
                    1
                )  # alpha
                eeg_freqSpec_band_storage[4, :, 0 : Sxx.shape[2], i_scenario] = Sxx[
                    :, 13:29, :
                ].mean(
                    1
                )  # beta
                eeg_freqSpec_band_storage[5, :, 0 : Sxx.shape[2], i_scenario] = Sxx[
                    :, 30:44, :
                ].mean(
                    1
                )  # gamma

                # since transitioning to csv the 4D matrices don't work out.
                # if file_types[i_seat] == "abm_leftseat":
                #     eeg_freqSpec_band_storage_df = pd.DataFrame(
                #         eeg_freqSpec_band_storage
                #     )
                #     eeg_freqSpec_band_storage_df.to_csv(
                #         "Processing/" + "eeg_freqSpec_band_storage_leftseat.csv"
                #     )
                ...
                #     )
                #     eeg_freqSpec_band_storage_df.to_csv(
                #         "Processing/" + "eeg_freqSpec_band_storage_rightseat.csv"
                #     )
                ...
                for this_epoch in range(eeg_freqSpec_band_storage.shape[2]):
                    if np.all(
                        np.isnan(
                            eeg_freqSpec_band_storage[:, :, this_epoch, i_scenario]
                        )
                    ):
                        number_of_time_epochs_spec = this_epoch - 1
                        break
                eeg_timesec_epoch_storage = eeg_freqSpec_band_storage[
                    0, 0, 0:number_of_time_epochs_spec, i_scenario
                ]
                x_axis_vector_spec = np.linspace(0, 100, number_of_time_epochs_spec)
                this_epoch_bandpower_ratio_spec = np.zeros(
                    (4, 9, number_of_time_epochs_spec)
                )
                engagement_index_spec = np.zeros(
                    (len(scenarios), number_of_time_epochs_spec)
                )
                filtered_engagement_index_spec = np.zeros(
                    (len(scenarios), number_of_time_epochs_spec)
                )
                filtered_taskLoad_index_spec = np.zeros(
                    (len(scenarios), number_of_time_epochs_spec)
                )
                taskLoad_index_spec = np.zeros(
                    (len(scenarios), number_of_time_epochs_spec)
                )

                print("Processing: " + crew_dir + " scenario " + scenarios[i_scenario])
                for idx in range(0, eeg_timesec_epoch_storage.shape[0]):
                    # print(idx)
                    if this_event_data[0, i_scenario] <= eeg_timesec_epoch_storage[idx]:
                        this_event1_epoch = np.floor(
                            (idx - 1) / (number_of_time_epochs_spec / 100)
                        )
                        break
                for idx in range(0, eeg_timesec_epoch_storage.shape[0]):
                    # print(idx)
                    if this_event_data[1, i_scenario] <= eeg_timesec_epoch_storage[idx]:
                        this_event2_epoch = np.floor(
                            (idx - 1) / (number_of_time_epochs_spec / 100)
                        )
                        break
                ...
                for this_epoch in range(this_epoch_bandpower_ratio_spec.shape[2]):
                    ...
                    this_epoch_bandpower_ratio_spec[
                        :, :, this_epoch
                    ] = eeg_freqSpec_band_storage[
                    ...
                    alpha = eeg_freqSpec_band_storage[
                        3, include_electrode_vector, this_epoch, i_scenario
                    ].sum()  # alpha
                    beta = eeg_freqSpec_band_storage[
                        4, include_electrode_vector, this_epoch, i_scenario
                    ].sum()  # beta
                    ...
                    engagement_index_spec[i_scenario, this_epoch] = per_power_beta / (
                        per_power_alpha + per_power_theta
                    )
                    # task load index
                ...
                this_eegTimeSeries_np[:, 0] = helper.getCrewInt(crews_to_process[i_crew])
                ...
                for this_epoch in range(int(number_of_time_epochs_spec)):
                    if (
                        (
                            eeg_timesec_epoch_storage[this_epoch]
                    ...
                    this_eegTimeSeries_np[this_epoch, 5] = taskLoad_index_spec[
                        i_scenario, this_epoch
                    ]
                    this_eegTimeSeries_np[this_epoch, 6] = engagement_index_spec[
                        i_scenario, this_epoch
                    ]
                ...
                event_eegTimeSeries_metrics = pd.concat(
                    [event_eegTimeSeries_metrics, this_eegTimeSeries_df]
                )

                if plot_individual:
                    this_epoch_bandpower_ratio_spec[
                        np.isnan(this_epoch_bandpower_ratio_spec)
                    ] = 0
                    ...
                    if plot_workload:
                        ...
                        if (crews_to_process[i_crew] == "Crew_12") & (
                            ...
                            expected_length_of_data = (
                                eeg_timesec_epoch_storage[3, -1] / 60
                            )
                            if len(rta_vector) != int(
                                np.floor(expected_length_of_data)
                            ):
                                print(
                                    "WARNING: RTA workload does not match trial length"
                                )
                                print(
                                    "rta = "
                                    + str(len(rta_vector))
                                    + " eeg: "
                                    + str(int(np.floor(expected_length_of_data)))
                                )
                        elif scenarios[i_scenario] == "2":
                            expected_length_of_data = (
                                eeg_timesec_epoch_storage[1, -1] / 60
                            )
                            if len(rta_vector) != int(
                                np.floor(expected_length_of_data)
                            ):
                            ...
                            rta_vector_timewarped = np.repeat(
                                rta_vector,
                                int(np.floor(number_of_epochs / len(rta_vector))),
                            )
                            if len(rta_vector_timewarped) < number_of_epochs:
                                diff = number_of_epochs - len(
                                    rta_vector_timewarped
                                )
                                rta_vector_timewarped = np.append(
                                    rta_vector_timewarped,
                                    np.ones(diff) * rta_vector_timewarped[-1],
                                )
                            axs[0, 0].plot(
                                x_axis_vector, rta_vector_timewarped * 10, linewidth=1
                            )
                            axs[0, 1].plot(
                                x_axis_vector, rta_vector_timewarped * 10, linewidth=1
                            )
