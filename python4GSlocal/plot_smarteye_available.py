...
total_smarteye_available_matrix = np.zeros((len(scenarios),2,len(crews_to_process)))

for i_crew in range(len(crews_to_process)):
	crew_dir = crews_to_process[i_crew]
	print(crew_dir)
	this_matrix = pd.read_table(('gs://soteria_study_data/'+ crew_dir + '/Processing/smarteye_pct_usable_matrix.csv'),delimiter=',')
...
fig.tight_layout()
# plt.show()
plt.savefig("Figures/" + 'total_smarteye_available_matrix.jpg')
