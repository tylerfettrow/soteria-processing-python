...
for i_crew in range(len(crews_to_process)):
	...
	for i_scenario in range(len(scenarios)):
		...
		if scenarios[i_scenario] == "3":
			...
			blob = bucket.blob(process_dir_name + 'ifd_cockpit_scenario' + scenarios[i_scenario] + '.csv')
			if blob.exists():
				ifd_data = pd.read_table(('gs://soteria_study_data/' + process_dir_name + 'ifd_cockpit_scenario' + scenarios[i_scenario] + '.csv'),delimiter=',')
